...
2. 큐에 들어있는 값을 순서대로 리스트에 보냄

3. 리스트에서 타겟이 되는 우선순위 보다 작은 수는 무시하고 버림 (리스트에는 타겟 우선순위 보다 우선순위가 같거나 큰 노드만 들어가게됨)

4. 원형리스트에 데이터를 다 추가하면 이제 가장 높은 우선순위의 노드부터 지우는 작업을 진행 (이때 지운 개수 cnt++)

5. 다 지우면 원형리스트에는 타겟 노드와 같은 우선순위는 노드만 남게됨.

6. 타겟 노드가 나올 때까지 남은 노드를 지우면서 개수를 세어줌 (cnt++)
