...
>
> 런타임 에러의 명확한 원인은 모르겠으나
>
> `배열에 할당된 크기를 넘어 접근하는 경우`, `전역 배열의 크기가 메모리 제한을 초과하는 경우`
>
> `지역 배열의 크기가 스택 크기 제한을 넘어가는 경우`, `0으로 나눌 때`, `라이브러리에서 예외를 발생시켰을 경우`
>
> `재귀 호출이 너무 깊어질 경우`, `이미 해제된 메모리를 또 참조할 경우`, `프로그램이 0이 아닌 수를 반환할 경우`
>
> 등이 있다고 한다... 나중에 `Node`에 관해 배우고, `분리연결법`을 구현할 수 있을 때 다시 도전해보도록 한다...!
